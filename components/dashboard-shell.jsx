...
import {
  ...
  Sun, Users, X, TrendingUp, Clock3, MoreHorizontal, ChevronLeft, ChevronRight
} from "lucide-react";
  ...
  return <div className={`${dark ? "admin-app dark" : "admin-app"} ${collapsed ? "sidebar-collapsed" : ""}`}>
    ...
    <Sheet>
      <div className="mobile-admin-bar">
        <SheetTrigger asChild><button className="icon-btn" aria-label="Open menu"><Menu size={21}/></button></SheetTrigger>
        <div className="mobile-brand"><span>☕</span> Cappuccino</div>
        <button className="icon-btn" onClick={() => setDark(!dark)} aria-label="Toggle theme">{dark ? <Sun size={19}/> : <Moon size={19}/>}</button>
      </div>
      <SheetContent side="left" className="mobile-sheet"><SheetTitle className="sr-only">Dashboard menu</SheetTitle><Sidebar onNavigate={(x) => notify(`${x} selected`)} /></SheetContent>
    </Sheet>
    ...
    {toast && <div className="admin-toast"><span>☕</span><div><strong>Cappuccino</strong><p>{toast}</p></div><button onClick={() => setToast("")} aria-label="Close notification"><X size={15}/></button></div>}
  </div>;
}
